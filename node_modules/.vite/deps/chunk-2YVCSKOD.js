...
function makeReactive(target2, shallow) {
  if (!isReadonly(target2)) {
    ...
    var ob = observe(target2, shallow);
    if (!ob) {
...
function createRef(rawValue, shallow) {
  ...
  def(ref2, "__v_isShallow", true);
  ref2.dep = defineReactive(ref2, "value", rawValue, null, shallow);
  return ref2;
}
...
function initAttrsProxy(vm) {
  if (!vm._attrsProxy) {
    ...
    syncSetupAttrs(proxy2, vm.$attrs, emptyObject, vm);
  }
...
function defineProxyAttr(proxy2, key, instance) {
  Object.defineProperty(proxy2, key, {
    enumerable: true,
    configurable: true,
    get: function() {
...
function updateChildComponent(vm, propsData, listeners, parentVnode, renderChildren) {
  ...
  if (vm._attrsProxy) {
    if (syncSetupAttrs(vm._attrsProxy, attrs2, prevVNode.data && prevVNode.data.attrs || emptyObject, vm)) {
...
function doWatch(source, cb, _a2) {
  ...
  if (isRef(source)) {
    ...
    getter = isArray(source) ? function() {
...
function observe(value, shallow) {
  ...
  if (hasOwn(value, "__ob__") && value.__ob__ instanceof Observer) {
    ob = value.__ob__;
  } else if (shouldObserve && !isServerRendering() && (isArray(value) || isPlainObject(value)) && Object.isExtensible(value) && !value.__v_skip) {
  ...
  if (el.staticRoot && !el.staticProcessed) {
    ...
    if (el.component) {
      ...
      if (bindings && bindings.__isScriptSetup !== false) {
        tag = checkBindingType(bindings, el.tag) || checkBindingType(bindings, camelize(el.tag)) || checkBindingType(bindings, capitalize(camelize(el.tag)));
      }
      if (!tag)
        tag = "'".concat(el.tag, "'");
...
function genFor(el, state, altGen, altHelper) {
  ...
  if (state.maybeComponent(el) && el.tag !== "slot" && el.tag !== "template" && !el.key) {
    state.warn(
      "<".concat(el.tag, ' v-for="').concat(alias, " in ").concat(exp, '">: component lists rendered with ') + "v-for should have explicit keys. See https://vuejs.org/guide/list.html#key for more info.",
...
var emptyObject, isArray, _toString, isBuiltInTag, isReservedAttribute, hasOwnProperty, camelizeRE, camelize, capitalize, hyphenateRE, hyphenate, bind$1, no, identity, SSR_ATTR, ASSET_TYPES, LIFECYCLE_HOOKS, config, unicodeRegExp, bailRE, hasProto, inBrowser, UA, isIE, isIE9, isEdge, isIOS, isFF, nativeWatch, supportsPassive, opts, _isServer, isServerRendering, devtools, hasSymbol, _Set, currentInstance, warn$2, tip, generateComponentTrace, formatComponentName, hasConsole_1, classifyRE_1, classify_1, repeat_1, __assign, uid$2, Dep, targetStack, VNode, createEmptyVNode, arrayProto, arrayMethods, methodsToPatch, RefFlag, rawToReadonlyFlag, rawToShallowReadonlyFlag, seenObjects, mark, measure, perf_1, normalizeEvent, initProxy, allowedGlobals_1, warnNonPresent_1, warnReservedPrefix_1, hasProxy_1, isBuiltInModifier_1, hasHandler_1, getHandler_1, sharedPropertyDefinition, computedWatcherOptions, activeEffectScope, EffectScope, uid$1, componentVNodeHooks, hooksToMerge, SIMPLE_NORMALIZE, ALWAYS_NORMALIZE, currentRenderingInstance, target$1, activeInstance, isUpdatingChildComponent, MAX_UPDATE_COUNT, queue, activatedChildren, has, circular, waiting, flushing, index$1, currentFlushTimestamp, getNow, performance_1, uid, Watcher, WATCHER, WATCHER_CB, WATCHER_GETTER, WATCHER_CLEANUP, INITIAL_WATCHER_VALUE, isUsingMicroTask, callbacks, pending, timerFunc, p_1, counter_1, observer, textNode_1, onBeforeMount, onMounted, onBeforeUpdate, onUpdated, onBeforeUnmount, onUnmounted, onErrorCaptured, onActivated, onDeactivated, onServerPrefetch, onRenderTracked, onRenderTriggered, version, arrayKeys, NO_INIITIAL_VALUE, shouldObserve, Observer, strats, defaultStrat, simpleCheckRE, functionTypeCheckRE, EXPLICABLE_TYPES, patternTypes, KeepAlive, builtInComponents, isReservedAttr, acceptValue, mustUseProp, isEnumeratedAttr, isValidContentEditableValue, convertEnumeratedValue, isBooleanAttr, xlinkNS, isXlink, getXlinkProp, isFalsyAttrValue, namespaceMap, isHTMLTag, isSVG, isPreTag, isReservedTag, unknownElementCache, isTextInputType, nodeOps, ref, emptyNode, hooks, directives$1, emptyModifiers, baseModules, attrs, klass$1, validDivisionCharRE, len, str, chr, index, expressionPos, expressionEndPos, warn$1, RANGE_TOKEN, CHECKBOX_RADIO_TOKEN, target, useMicrotaskFix, events, svgContainer, domProps, parseStyleText, cssVarRE, importantRE, setProp, vendorNames, emptyStyle, normalize, style$1, whitespaceRE$1, autoCssTransition, hasTransition, TRANSITION, ANIMATION, transitionProp, transitionEndEvent, animationProp, animationEndEvent, raf, transformRE, transition, platformModules, modules$1, patch, directive, show, platformDirectives, transitionProps, isNotTextNode, isVShowDirective, Transition, props, TransitionGroup, platformComponents, defaultTagRE, regexEscapeRE, buildRegex, klass, style, decoder, he, isUnaryTag, canBeLeftOpenTag, isNonPhrasingTag, attribute, dynamicArgAttribute, ncname, qnameCapture, startTagOpen, startTagClose, endTag, doctype, comment, conditionalComment, isPlainTextElement, reCache, decodingMap, encodedAttr, encodedAttrWithNewLines, isIgnoreNewlineTag, shouldIgnoreFirstNewline, onRE, dirRE, forAliasRE, forIteratorRE, stripParensRE, dynamicArgRE, argRE, bindRE, modifierRE, slotRE, lineBreakRE, whitespaceRE, invalidAttributeRE, decodeHTMLCached, emptySlotScopeToken, warn, delimiters, transforms, preTransforms, postTransforms, platformIsPreTag, platformMustUseProp, platformGetTagNamespace, maybeComponent, ieNSBug, ieNSPrefix, model, modules, directives, baseOptions, isStaticKey, isPlatformReservedTag, genStaticKeysCached, fnExpRE, fnInvokeRE, simplePathRE, keyCodes, keyNames, genGuard, modifierCode, baseDirectives, CodegenState, prohibitedKeywordRE, unaryOperatorsRE, stripStringRE, range, createCompiler, _a, compileToFunctions, div, shouldDecodeNewlines, shouldDecodeNewlinesForHref, idToTemplate, mount;
var init_vue_esm = __esm({
  "node_modules/vue/dist/vue.esm.js"() {
    ...
    camelizeRE = /-(\w)/g;
    camelize = cached(function(str2) {
      return str2.replace(camelizeRE, function(_, c) {
        return c ? c.toUpperCase() : "";
      });
    });
    capitalize = cached(function(str2) {
      return str2.charAt(0).toUpperCase() + str2.slice(1);
    });
    hyphenateRE = /\B([A-Z])/g;
    hyphenate = cached(function(str2) {
      return str2.replace(hyphenateRE, "-$1").toLowerCase();
    });
    bind$1 = Function.prototype.bind ? nativeBind : polyfillBind;
    no = function(a, b, c) {
      return false;
    };
    identity = function(_) {
    ...
    createEmptyVNode = function(text2) {
      if (text2 === void 0) {
        text2 = "";
      }
      var node = new VNode();
    ...
    if (true) {
      ...
      warnReservedPrefix_1 = function(target2, key) {
        warn$2('Property "'.concat(key, '" must be accessed with "$data.').concat(key, '" because ') + 'properties starting with "$" or "_" are not proxied in the Vue instance to prevent conflicts with Vue internals. See: https://vuejs.org/v2/api/#data', target2);
      };
    ...
    Observer = /** @class */
    function() {
      function Observer2(value, shallow) {
        if (shallow === void 0) {
          shallow = false;
        }
        this.value = value;
      ...
      Observer2.prototype.walk = function(obj, shallow) {
        var keys = Object.keys(obj);
        for (var i = 0; i < keys.length; i++) {
          var key = keys[i];
          defineReactive(obj, key, NO_INIITIAL_VALUE, void 0, shallow);
        }
      };
      Observer2.prototype.observeArray = function(items) {
    ...
    platformModules = [attrs, klass$1, events, domProps, style$1, transition];
    modules$1 = platformModules.concat(baseModules);
    patch = createPatchFunction({ nodeOps, modules: modules$1 });
    if (isIE9) {
    ...
    defaultTagRE = /\{\{((?:.|\r?\n)+?)\}\}/g;
    regexEscapeRE = /[-.*+?^${}()|[\]\/\\]/g;
    buildRegex = cached(function(delimiters2) {
    ...
    ncname = "[a-zA-Z_][\\-\\.0-9_a-zA-Z".concat(unicodeRegExp.source, "]*");
    qnameCapture = "((?:".concat(ncname, "\\:)?").concat(ncname, ")");
    startTagOpen = new RegExp("^<".concat(qnameCapture));
    startTagClose = /^\s*(\/?)>/;
    endTag = new RegExp("^<\\/".concat(qnameCapture, "[^>]*>"));
    doctype = /^<!DOCTYPE [^>]+>/i;
    comment = /^<!\--/;
    conditionalComment = /^<!\[/;
    ...
    shouldIgnoreFirstNewline = function(tag, html2) {
    ...
    decodeHTMLCached = cached(he.decode);
    ...
    genStaticKeysCached = cached(genStaticKeys);
    ...
    prohibitedKeywordRE = new RegExp("\\b" + "do,if,for,let,new,try,var,case,else,with,await,break,catch,class,const,super,throw,while,yield,delete,export,import,return,switch,default,extends,finally,continue,debugger,function,arguments".split(",").join("\\b|\\b") + "\\b");
    unaryOperatorsRE = new RegExp("\\b" + "delete,typeof,void".split(",").join("\\s*\\([^\\)]*\\)|\\b") + "\\s*\\([^\\)]*\\)");
    stripStringRE = /'(?:[^'\\]|\\.)*'|"(?:[^"\\]|\\.)*"|`(?:[^`\\]|\\.)*\$\{|\}(?:[^`\\]|\\.)*`|`(?:[^`\\]|\\.)*`/g;
    range = 2;
...
vue/dist/vue.esm.js:
   ...
   *)
*/
//# sourceMappingURL=chunk-2YVCSKOD.js.map
